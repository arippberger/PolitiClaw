...
| --- | --- | --- | --- | --- |
| [`politiclaw_set_preferences`](./tools/politiclaw_set_preferences.md) | Preferences and onboarding | `core` | `lead-in-guides` | Essential first-run setup that unlocks reps, ballot lookup, and election prep. |
| [`politiclaw_get_preferences`](./tools/politiclaw_get_preferences.md) | Preferences and onboarding | `internal` | `generated-reference-only` | State inspection helper, useful for debugging but rarely a user goal by itself. |
| [`politiclaw_record_stance_signal`](./tools/politiclaw_record_stance_signal.md) | Preferences and onboarding | `internal` | `generated-reference-only` | Low-level preference telemetry for flows and monitoring, not a normal direct user action. |
| [`politiclaw_set_issue_stance`](./tools/politiclaw_set_issue_stance.md) | Preferences and onboarding | `core` | `lead-in-guides` | Directly expresses user priorities and powers bill and rep alignment. |
| [`politiclaw_list_issue_stances`](./tools/politiclaw_list_issue_stances.md) | Preferences and onboarding | `advanced` | `follow-up-or-advanced-docs` | Useful for reviewing and tuning saved preferences after onboarding. |
| [`politiclaw_delete_issue_stance`](./tools/politiclaw_delete_issue_stance.md) | Preferences and onboarding | `advanced` | `follow-up-or-advanced-docs` | Cleanup control for users editing their stance set, but not part of the default flow. |
| [`politiclaw_set_monitoring_cadence`](./tools/politiclaw_set_monitoring_cadence.md) | Preferences and onboarding | `core` | `lead-in-guides` | Best user-facing control for how loud ongoing monitoring should be. |
| [`politiclaw_start_onboarding`](./tools/politiclaw_start_onboarding.md) | Preferences and onboarding | `core` | `lead-in-guides` | Primary setup entry point that hides several lower-level preference tools behind one flow. |
...
| [`politiclaw_download_shapefiles`](./tools/politiclaw_download_shapefiles.md) | Representatives and alignment | `internal` | `generated-reference-only` | Operational helper for the zero-key rep lookup path, not something most users should think about. |
...
| [`politiclaw_ingest_house_votes`](./tools/politiclaw_ingest_house_votes.md) | Bills and votes | `internal` | `generated-reference-only` | Data-ingestion plumbing that supports rep scoring but is not a user-facing civic task. |
...
| [`politiclaw_setup_monitoring`](./tools/politiclaw_setup_monitoring.md) | Monitoring and cadence | `internal` | `generated-reference-only` | Operator-style reconciliation tool that is superseded for most users by cadence selection. |
| [`politiclaw_pause_monitoring`](./tools/politiclaw_pause_monitoring.md) | Monitoring and cadence | `internal` | `generated-reference-only` | Administrative control for installed jobs, not the preferred everyday monitoring UX. |
| [`politiclaw_resume_monitoring`](./tools/politiclaw_resume_monitoring.md) | Monitoring and cadence | `internal` | `generated-reference-only` | Administrative control for installed jobs, not the preferred everyday monitoring UX. |
