...
Compute how much a federal bill touches the user's declared issue stances. Deterministic (no LLM): matches policy area, subjects, title, and summary against each declared stance. Reports relevance and confidence; confidence below the 0.4 floor renders as "insufficient data". Rationale names specific matched subjects (never abstract generalities). Requires declared issue stances (see politiclaw_set_issue_stance) and plugins.politiclaw.apiKeys.apiDataGov for the bill source.
